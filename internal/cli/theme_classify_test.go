...
func TestThemeClassifyRealFilesystemEntriesTextAndJSON(t *testing.T) {
	...
	for _, want := range []string{"Path: src/main.go", "Type: file", "Kind: source.go", "Roles: source", "Matched by: extension (.go)", "Theme: vivid (built-in)", "Text: color=#66F0C0", "Icon: ascii=\"[SC]\" unicode=\"•\" nerd=\"󰟓\" color=#00FFD1"} {
