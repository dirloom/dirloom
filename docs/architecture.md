...
- `internal/config`: strict YAML parsing, project and user discovery, layered resolution, provenance and diagnostics.
...
Configuration is resolved before the application service starts scanning. The resolver is independent from Cobra, distinguishes omitted and explicit zero values, and returns a complete effective request. Project discovery is bounded by the nearest Git worktree; configuration never selects the inspected root or an output path.
